...
export default function PlansModal({ isOpen, onClose, currentPlanName, whatsappNumber }: PlansModalProps) {
  ...
  const getFeaturesList = (plan: Plan) => {
    ...
    features.push(`${formatLimit(plan.features.maxSpaces)} Space${plan.features.maxSpaces !== 1 ? 's' : ''}`);
    features.push(`${formatLimit(plan.features.maxLists)} List${plan.features.maxLists !== 1 ? 's' : ''}`);
    features.push(`${formatLimit(plan.features.maxFolders)} Folder${plan.features.maxFolders !== 1 ? 's' : ''}`);
    ...
    if (plan.features.hasGroupChat) {
      features.push('Group Chat');
      features.push(`${formatLimit(plan.features.messageLimit)} Messages/month`);
    }
